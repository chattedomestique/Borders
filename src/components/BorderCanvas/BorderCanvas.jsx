 ...
 */
function drawFrostedBg(ctx, source, canvasW, canvasH) {
  // Draw blurred bg — use a small intermediary for performance
  const tmp = document.createElement('canvas')
  ...
  ctx.save()
  ctx.filter = 'blur(24px) saturate(1.6) brightness(0.85)'
  // Scale to cover
...
function renderFrame(canvas, source, settings) {
  ...
  // 1. Draw background
  if (bgMode === 'frosted') {
    drawFrostedBg(ctx, source, totalW, totalH)
  } else {
...
const BorderCanvas = forwardRef(function BorderCanvas({ media, settings }, ref) {
  ...
  // Keep settingsRef synced without re-running effects
  ...
  // For video: continually render frames while media is playing (video preview is static)
  // We show a static first-frame preview, which is fine.
